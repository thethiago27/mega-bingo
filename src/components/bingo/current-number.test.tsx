...
describe('CurrentNumber Component', () => {
  ...
  it('should display countdown timer when nextDrawIn is provided', () => {
    vi.spyOn(useRoomModule, 'useRoom').mockReturnValue({
      currentNumber: 42,
    } as ReturnType<typeof useRoomModule.useRoom>);

    render(<CurrentNumber roomId="test-room" nextDrawIn={5} />);

    expect(screen.getByText('Sorteando próximo em 5s...')).toBeInTheDocument();
  });

  it('should not display countdown timer when nextDrawIn is not provided', () => {
    vi.spyOn(useRoomModule, 'useRoom').mockReturnValue({
      currentNumber: 42,
    } as ReturnType<typeof useRoomModule.useRoom>);
  ...
  it('should display different countdown values', () => {
    ...
    const { rerender } = render(
      <CurrentNumber roomId="test-room" nextDrawIn={10} />
    );
    expect(screen.getByText('Sorteando próximo em 10s...')).toBeInTheDocument();

    rerender(<CurrentNumber roomId="test-room" nextDrawIn={3} />);
