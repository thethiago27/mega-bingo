...
export default function BingoButton() {
  ...
  const { cardNumbers, markedNumbers } = useRoom(roomId);

  const handleBingo = () => {
    if (markedNumbers.length === cardNumbers.length) {
      alert('🎉 BINGO! Você ganhou!');
    }
  };

  const bingoComplete =
    markedNumbers.length === cardNumbers.length && cardNumbers.length > 0;
