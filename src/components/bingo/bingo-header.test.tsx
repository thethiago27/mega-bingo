...
describe('BingoHeader Component', () => {
  ...
  it('should render the header with room ID', () => {
    render(<BingoHeader roomId="ABC123" status={RoomStatus.IN_PLAY} />);
  ...
  it('should display finished status', () => {
    render(<BingoHeader roomId="ABC123" status={RoomStatus.FINISHED} />);
  ...
  it('should have sticky positioning and backdrop blur', () => {
    const { container } = render(
      <BingoHeader roomId="ABC123" status={RoomStatus.IN_PLAY} />
    );
  ...
  it('should display room ID with different values', () => {
    const { rerender } = render(
      <BingoHeader roomId="ROOM1" status={RoomStatus.IN_PLAY} />
    );
    expect(screen.getByText('Sala #ROOM1')).toBeInTheDocument();

    rerender(<BingoHeader roomId="ROOM2" status={RoomStatus.FINISHED} />);
